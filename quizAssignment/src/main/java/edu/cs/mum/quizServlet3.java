...
@WebServlet("/quiz4")
public class quizServlet3 extends HttpServlet {
    String[] pi = {"3,1,4,1,5"};
    ...
    String nextQuestion = powerOfTwo.getQuestions();
    protected void doGet(HttpServletRequest req, HttpServletResponse res)
            throws ServletException, IOException {
        res.setContentType("text/html");
        PrintWriter out = res.getWriter();
        int answer_input = Integer.parseInt(req.getParameter("answer4"));
        int counter = (Integer)req.getServletContext().getAttribute("counter");
        if(answer_input == pri.getAnswers()){
            counter++;
        }
        out.println("<body>\n" +
                ...
                "<span>Your Answer</span> <input type=\"number\" name=\"answer5\" >\n" +
                "<div>\n" +
